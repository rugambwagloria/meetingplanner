...
public class PlannerInterface {
    ...
    public void mainMenu() {
        ...
        //Get user input - throws immediately on invalid
        int userInput = readInt("Please press the number that corresponds to what you would like to do.", 0, 6, "menu option");
    ...
    private int readInt(String prompt, int min, int max, String type) {
        String input = inputOutput(prompt).trim();
        try {
            int value = Integer.parseInt(input);
            if (value < min || value > max) {
                throw new IllegalArgumentException(String.format("Invalid %s: %d (must be %d-%d)", type, value, min, max));
            }
            return value;
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Invalid number for " + type + ": '" + input + "'", e);
        }
    }
    ...
    private int readMonth(String prompt) {
        while (true) {
            try {
                return readInt(prompt, 1, 12, "month");
    ...
    private int readDay(String prompt, int month) {
        while (true) {
            int maxDay = getDaysInMonth(month);
            try {
                return readInt(prompt, 1, maxDay, "day");
            } catch (IllegalArgumentException e) {
                System.out.println(e.getMessage());
            }
        }
    ...
    private int readHour(String prompt) {
        while (true) {
            try {
                return readInt(prompt, 0, 23, "hour");
            } catch (IllegalArgumentException e) {
                System.out.println(e.getMessage());
            }
        }
    ...
    /**
     * Allows the user to schedule a meeting by entering information about the
     * meeting.
     */
    public void scheduleMeeting() {
        ...
        //Read in room ID
        String id = inputOutput("\nEnter the desired room ID, or cancel to cancel: ").trim();
        if (id.equals("cancel")) {
            this.mainMenu();
            return;
        }
    ...
    public void scheduleVacation() {
        ...
        //Read in start month
        int sMonth = readMonth("\nEnter the month that the vacation starts (1-12): ");

        //Read in start day
        int sDay = readDay("\nEnter the day the vacation starts (1-31): ", sMonth);

        //Read in end month
        int eMonth = readMonth("\nEnter the month that the vacation ends (1-12): ");

        //Read in end day
        int eDay = readDay("\nEnter the day the vacation ends (1-31): ", eMonth);
    ...
    /**
     * Allows the user to check the availability of a given employee at a
     * particular time.
     */
    ...
    /**
     * Allows the user to view the agenda of a room on a particular month or
     * day.
     */
    public void checkAgendaRoom() {
        ...
        //Read in day
        String day = inputOutput("\nEnter the day (1-31), or all to see the whole month: ").trim();
        ...
        //Read in room ID
        String name = inputOutput("\nEnter a room ID, or cancel to cancel the request: ").trim();
        if (!name.equals("cancel")) {
            try {
    ...
    /**
     * Allows the user to view the agenda for a particular employee on a
    ...
    public void checkAgendaPerson() {
        ...
        //Read in name
        String name = inputOutput("\nEnter a name, or cancel to cancel the request: ").trim();
        if (!name.equals("cancel")) {
            try {
                Person who = org.getEmployee(name);
                if (!dayStr.equals("all")) {
                    int day = Integer.parseInt(dayStr);
                    System.out.println(who.printAgenda(month, day));
                } else {
                    System.out.println(who.printAgenda(month));
                }
            } catch (Exception e) {
